...
export class PaymentPageComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private dataservice: DataserviceService, private http: HttpClient, private busservice: BusService, private carbonFootprintCalculator: CarbonFootprintCalculatorComponent) { }
  ...
  calculateCarbonFootprint() {
    this.carbonFootprintCalculator.distance = this.distance;
    this.carbonFootprintCalculator.modeOfTravel = this.modeOfTravel;
    this.carbonFootprintCalculator.calculateCarbonFootprint();
    this.carbonFootprint = this.carbonFootprintCalculator.carbonFootprint;
    this.incentive = this.carbonFootprintCalculator.incentive;
    this.couponCode = this.carbonFootprintCalculator.couponCode;
  }

  applyCoupon() {
    if (this.couponCode) {
      // Apply discount based on the coupon code
      if (this.incentive.includes('10%')) {
        this.passfare *= 0.9;
      } else if (this.incentive.includes('5%')) {
        this.passfare *= 0.95;
      }
    }
  }
